...
| Name | Value | Role |
|------|-------|------|
| Canvas White | `#ffffff` | Page background, primary card surfaces, popovers. The foundational bright base. |
| Ghost Gray | `#f2f2f2` | Secondary background for segmented sections or subtle card differentiation. Lighter than default background. |
| Subtle Ash | `#e5e5e5` | Border colors for inputs, cards, and dividers. Provides definition without harshness. |
| Midtone Gray | `#737373` | Muted text, placeholder text in inputs, secondary icons. Recedes into the background. |
| Rich Black | `#0a0a0a` | Primary text color for body copy, standard icons, badges with white text. High contrast for readability. |
| Deep Black | `#000000` | Headings, active state button backgrounds, highlighted text. The darkest tone for strong emphasis. |
| Callout Red | `#c22b10` | Destructive actions, error states. A muted, serious red. |
| Success Green | `#10c22b` | Success states, positive confirmations. A muted, serious green. |

## Typography

### Geist — Primary brand font for all UI text, headings, and body. Its varied weights and precise tracking create a modern, technical feel.
- **Substitute:** Inter
- **Weights:** 400, 500, 600
...
### Geist Mono — Used for code snippets or specific input fields requiring monospaced characters. Reinforces a technical aesthetic.
...
| caption | 12px | 1.5 | — |
| body | 14px | 1.43 | — |
| heading | 18px | 1.33 | -0.45px |
| display | 48px | 1 | -2.4px |
...
- **Elevated Card** (`#ffffff`) — Content cards and distinct sections that require a subtle lift, defined by borders or minimal shadow.
- **Search/Input Field** (`#ffffff`) — Interactive elements like search bars and inputs, often bordered.
- **Popovers/Overlays** (`#ffffff`) — Transient UI elements that appear above other content.
- **Ghost Gray Background** (`#f2f2f2`) — Used as a background color for secondary buttons or badges, indicating a slightly lower hierarchy.
