...
export default function EditTaskDrawer({ task }: { task: Task }) {
        ...
        <>
          ...
          <div className="fixed inset-y-0 right-0 z-50 w-full max-w-md bg-white flex flex-col shadow-2xl">
            ...
            <div className="flex items-center justify-between px-6 py-4 border-b border-[#e5e5e5] shrink-0">
              <h2 className="text-[16px] font-semibold text-[#000000]">Edit Task</h2>
            ...
            <form ref={formRef} onSubmit={handleSubmit} className="flex-1 overflow-y-auto px-6 py-5 flex flex-col gap-5">
              <Field label="Task Title" name="title"     required defaultValue={task.title}     placeholder="e.g. Follow up with client" />
              <Field label="Related To" name="relatedTo"          defaultValue={task.relatedTo} placeholder="e.g. John Smith / Quote #001" />
              <Field label="Due Date"   name="dueDate"   type="date" required defaultValue={task.dueDate} />
              ...
              <div className="border-t border-[#e5e5e5] pt-4">
                {confirmDel ? (
                  <div className="flex flex-col gap-2">
                    <p className="t-caption text-[#737373]">This will permanently delete this task.</p>
